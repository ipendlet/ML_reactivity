...
class NaturalBondOrbital:
    def __init__(self, nboLines, atomsList):
        ...
        if firstLine[2].startswith('LP*'):
            # if the type is 3 characters long there isn't a space after it
            firstLine[2:3] = ['LP*','(']
        self.type = firstLine[2]

        # extract the atom indices from the first line of the NBO output
        if self.type != 'LP' and self.type !='LP*':
            self.atom1Index = int(firstLine[6][:-1]) - 1
            self.atom2Index = int(firstLine[8]) - 1
        elif self.type == 'LP' or self.type == 'LP*':
            self.atom1Index = int(firstLine[6]) - 1
            self.atom2Index = -1 # In other words no atom
       
        self.atomicNum1 = atomsList[self.atom1Index].atomicNum
        self.atom1Charge = atomsList[self.atom1Index].naturalCharge
        ...
        if self.type != 'LP' and self.type != 'LP*':
            self.atomicNum2 = atomsList[self.atom2Index].atomicNum
            self.atom2Charge = atomsList[self.atom2Index].naturalCharge
    ...
    def get_feature_vector(self):
           return ([self.type, self.atomicNum1, self.atom1Charge, self.atomicNum2, self.atom2Charge,
                   self.orbitalRatioAtom1] + self.hybrids, (self.atom1Index + 1, self.atom2Index + 1))
    ...
    for i, line in enumerate(allNBOLines[1:], start=1):
        ...
        if line[:5].strip() or i + 1 == len(allNBOLines):
            ...
            if any(type in allNBOLines[lastIndex] for type in ['BD', 'LP']):
                one, two = NaturalBondOrbital(allNBOLines[lastIndex:i],atomsList).get_feature_vector()
                featureVector.append(one)
                bond_pairs.append(two)
            lastIndex = i
#    print(featureVector)
    return(featureVector, bond_pairs)
